...
namespace BitSits_Framework
{
    ...
    class Army
    {
        ...
        const float MaxRemoveTime = 1.5f, MaxPathVisibleTime = .8f;

        readonly float Damage, MaxHealth, MaxReloadTime, Range, MoveSpeed, defaultRotaion;
        float health, reloadTime, pathVisibleTime, rotation, removeTime = MaxRemoveTime + .3f;
        ...
        {
            nextDestPosition.Y = MathHelper.Clamp(nextDestPosition.Y, 0, Camera2D.BaseScreenSize.Y - 1);
            nextDestPosition.X = MathHelper.Clamp(nextDestPosition.X, 0, Camera2D.BaseScreenSize.X - 1);
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (destination.Count > 0)
            {
                ...
                rotation = theta;
                position +=  new Vector2((float)Math.Cos(theta), (float)Math.Sin(theta)) * MoveSpeed
                    * (float)gameTime.ElapsedGameTime.TotalSeconds * 40;
        ...
        public void EnemyAI(Rectangle enemyCastleBounds)
        {
            if (destination.Count == 0)
            {
                ...
                {
                    int x = gameContent.random.Next((int)Camera2D.BaseScreenSize.X - 1);
                    int y = gameContent.random.Next((int)Camera2D.BaseScreenSize.Y - 1);
        ...
        public void HandleEnemy(List<Army> armies)
        {
            ...
            if (nearestEnemy != null && D < Range && reloadTime > MaxReloadTime)
            {
                ...
                gameContent.noise[(int)Rank].Play();
            }
        }

        public void DrawPath(GameTime gameTime, SpriteBatch spriteBatch)
        {
            Color c = Color.White * (pathVisibleTime / MaxPathVisibleTime);
        ...
        public void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            Color c = Color.White * (removeTime / MaxRemoveTime);

            animationPlayer.Draw(gameTime, spriteBatch, position, c, 0,
